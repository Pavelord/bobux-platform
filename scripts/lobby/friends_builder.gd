...
static func _run_avatar_preview_queue() -> void:
	...
	while not _avatar_preview_queue.is_empty():
		...
		var delay_seconds: float = float(job.get("delay_seconds", 0.0))
		...
		for _frame_index in range(3):
			await host.get_tree().process_frame
		await host.get_tree().create_timer(AVATAR_PREVIEW_RENDER_SETTLE_SECONDS).timeout
...
static func _create_avatar_render_viewport(render_host: Node, profile: Dictionary) -> SubViewport:
	...
	camera.fov = 27.0
	var camera_position := Vector3(0.0, 2.9, 5.55)
	var camera_target := Vector3(0.0, 2.42, 0.0)
	...
	avatar.set_meta("bobux_skip_custom_character_mesh", true)

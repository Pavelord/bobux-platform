...
const HOME_FRIENDS_RENDER_LIMIT: int = 8
...
func _build_home_dashboard() -> void:
	...
	greeting_label.text = uname
	greeting_label.add_theme_color_override("font_color", Color(0.15, 0.15, 0.15, 1))
	greeting_label.add_theme_font_size_override("font_size", 42)
	header_name_box.add_child(greeting_label)
	
	var sep_space := Control.new()
	sep_space.custom_minimum_size = Vector2(0, 10)
	home_content.add_child(sep_space)

	var friends_panel_parts := _create_dashboard_panel(home_content, "Friends", "See who is playing right now.", 260, "See All", Callable(self, "_open_home_see_all_friends"))
	...
	home_friends_clip.custom_minimum_size = Vector2(0, 172)
	...
	home_friends_container.add_theme_constant_override("separation", 18)
	...
	var recommended_panel_parts := _create_dashboard_panel(home_content, "Recommended", "Experiences with players online right now.", 270.0, "See All", Callable(self, "_open_home_see_all_games"))
	...
	var continue_panel_parts := _create_dashboard_panel(home_content, "Continue Playing", "Your recent games and maps.", 310.0, "See All", Callable(self, "_open_home_see_all_games"))
	...
	var creator_panel_parts := _create_dashboard_panel(home_content, "От создателей Bobux", "Режимы только от pavelord.", 300.0, "See All", Callable(self, "_open_home_see_all_games"))
	...
	if creator_panel_control != null:
		creator_panel_control.custom_minimum_size = Vector2(0, 270)
	...
	var new_panel_parts := _create_dashboard_panel(home_content, "New Experiences", "Freshly published places.", 270.0, "See All", Callable(self, "_open_home_see_all_games"))
	...
	var all_panel_parts := _create_dashboard_panel(home_content, "All Experiences", "Everything currently published on Bobux.", DISCOVER_PANEL_MIN_HEIGHT, "See All", Callable(self, "_open_home_see_all_games"))
...
func _open_create_game_choice_dialog() -> void:
	...
	dialog.title = "Choose editor"
	dialog.dialog_text = "Which game editor do you want to open?"
	dialog.ok_button_text = "New editor"
	dialog.cancel_button_text = "Cancel"
	dialog.exclusive = true
	dialog.add_button("Old editor", false, "legacy")
	dialog.confirmed.connect(func() -> void:
		dialog.queue_free()
		_show_template_picker_page()
	)
	dialog.custom_action.connect(func(action: StringName) -> void:
		if String(action) != "legacy":
			return
		dialog.queue_free()
		_open_old_place_editor_direct()
	)
...
func _show_template_picker_page(editor_scene_path: String = "res://scenes/place_editor/studio.tscn") -> void:
	...
	root.add_child(_create_develop_page_header("Choose a Template", "Pick a starting point for the old editor." if old_editor_mode else "Pick a starting point for a new Bobux experience.", true))
...
func _show_my_creations_page() -> void:
	...
	root.add_theme_constant_override("separation", 18)
	margin.add_child(root)
	root.add_child(_create_develop_page_header("My Creations", "Your Bobux experiences. Local files are drafts; published games must exist in cloud/server.", true))
	...
	hub_btn.text = "Develop Hub"
	hub_btn.custom_minimum_size = Vector2(130, 36)
	...
	for cloud_map_variant in my_creations_payload.get("maps", []):
		...
		if not bool(cloud_map.get("is_published", true)):
			continue
	...
	if model_count == 0:
		_add_placeholder_label(models_list, "No model drafts yet. Use Create Model to build or import one.")
	...
	var create_avatar_item_btn := Button.new()
	create_avatar_item_btn.text = "+ Create Avatar Item"
	create_avatar_item_btn.custom_minimum_size = Vector2(180, 36)
	create_avatar_item_btn.pressed.connect(_show_avatar_item_creator_page)
	avatar_box.add_child(create_avatar_item_btn)
...
func _apply_avatar_item_source_fallback_colors(loaded_model: Node, fallback_parts: Array) -> void:
	...
	for index in range(meshes.size()):
		...
		if part_data.is_empty() or not _avatar_item_part_has_color(part_data):
			continue
		if not _avatar_item_mesh_needs_fallback_color(mesh_instance):
			continue
		var fallback_material := _avatar_item_preview_material(_avatar_item_color_to_html(_avatar_item_part_color_value(part_data)))
...
func _avatar_item_mesh_needs_fallback_color(mesh_instance: MeshInstance3D) -> bool:
	...
	for surface_index in range(surface_count):
		...
		if material is BaseMaterial3D:
			...
			if color.a > 0.01 and (absf(color.r - 1.0) > 0.08 or absf(color.g - 1.0) > 0.08 or absf(color.b - 1.0) > 0.08):
				return false
	return true
...
func _create_cloud_place_catalog_card(map_entry: Dictionary) -> Control:
	...
	delete_btn.add_theme_color_override("font_color", Color(0.9, 0.16, 0.16))
...
func _create_model_draft_row(model_entry: Dictionary) -> Control:
	...
	author.add_theme_color_override("font_color", Color(0.52, 0.52, 0.52))
...
func _create_develop_page_header(title_text: String, subtitle_text: String, include_back: bool) -> Control:
	var header := HBoxContainer.new()
	...
	title.add_theme_color_override("font_color", Color(0.10, 0.10, 0.10))
	title_box.add_child(title)
	var subtitle := Label.new()
	subtitle.text = subtitle_text
	subtitle.add_theme_font_size_override("font_size", 14)
	subtitle.add_theme_color_override("font_color", Color(0.42, 0.42, 0.42))
...
func _create_template_picker_card(template_info: Dictionary, editor_scene_path: String = "res://scenes/place_editor/studio.tscn") -> Panel:
	...
	var style := _make_white_panel_style()
	...
	desc.add_theme_color_override("font_color", Color(0.42, 0.42, 0.42))
...
func _create_home_header_avatar_preview(size_px: int = 140) -> Control:
	...
	var cam := Camera3D.new()
	cam.fov = 26.0
	var cam_pos := Vector3(0.0, 3.1, 7.8)
	var cam_target := Vector3(0.0, 2.55, 0.0)
	cam.transform = Transform3D(Basis.looking_at(cam_target - cam_pos, Vector3.UP), cam_pos)
	svp.add_child(cam)

	var light := DirectionalLight3D.new()
	light.light_energy = 1.25
	light.transform = Transform3D(Basis.looking_at(Vector3(0, 2.4, 0) - Vector3(3.0, 6.0, 4.0), Vector3.UP), Vector3(3.0, 6.0, 4.0))
	svp.add_child(light)
	...
	if player_res:
		var p_avatar = player_res.instantiate()
		p_avatar.set("is_ui_preview", true)
		p_avatar.set("use_local_avatar_fallback", true)
		p_avatar.set("render_avatar_clothing_decals", true)
		p_avatar.position.y = -0.28
		p_avatar.name = "HomePreviewPlayer"
		p_avatar.head_color = GameState.head_color
		p_avatar.torso_color = GameState.torso_color
		p_avatar.left_arm_color = GameState.left_arm_color
		p_avatar.right_arm_color = GameState.right_arm_color
		p_avatar.left_leg_color = GameState.left_leg_color
		p_avatar.right_leg_color = GameState.right_leg_color
		var name_label: Node = p_avatar.get_node_or_null("Visuals/NameLabel")
		if name_label:
			name_label.visible = false
		svp.add_child(p_avatar)
		home_avatar_player = p_avatar
	else:
...
func _open_profile_for_user(user_id: String, username: String, profile_snapshot: Dictionary = {}, active_server: Dictionary = {}) -> void:
	...
	var own_user_id: String = str(UserSession.user_id).strip_edges()
	var own_username: String = str(UserSession.username).strip_edges().to_lower()
	var selected_username_lc: String = selected_profile_username.strip_edges().to_lower()
	var is_own_profile := (not selected_profile_user_id.is_empty() and selected_profile_user_id == own_user_id) or (selected_profile_user_id.is_empty() and not selected_username_lc.is_empty() and selected_username_lc == own_username)
	if not is_own_profile:
		_show_external_profile_popup(selected_profile_snapshot, selected_profile_active_server)
		call_deferred("_refresh_external_profile_popup_async", selected_profile_user_id, selected_profile_username, selected_profile_active_server.duplicate(true))
		return
...
func _refresh_selected_profile_async() -> void:
	...
	var avatar_outfit: Dictionary = avatar_outfit_result.get("data", {}) if bool(avatar_outfit_result.get("ok", false)) and avatar_outfit_result.get("data", {}) is Dictionary else {}
	if is_own_profile:
...
func _clear_home_game_sections() -> void:
	_clear_children_immediate(recommended_cards_grid)
	_clear_children_immediate(new_cards_grid)
	_clear_children_immediate(all_cards_grid)
...
func _render_home_game_sections(entries: Array, refresh_token: int) -> void:
	...
	var recommended_limit: int = maxi(HOME_RECOMMENDED_RENDER_LIMIT, compact_columns * 2)
	recommended_entries = _fill_home_entries_to_limit(recommended_entries, all_entries, recommended_limit)
...
func _refresh_published_game_cache(force: bool = false) -> void:
	...
	if not force and now_msec < _published_game_cache_expires_at_msec and _published_game_cache_loaded:
		return
	var maps_result: Dictionary = await CloudAPI.fetch_published_maps(128)
...
func _build_discover_entries() -> Array:
	...
	if CloudAPI.is_configured():
		...
		if bool(maps_result.get("ok", false)):
			published_maps = CloudAPI._extract_array_payload(maps_result.get("data", []))
...
func _get_cached_published_map_records() -> Array:
	...
	if cache.is_empty():
		return []
	var saved_at_msec: int = int(cache.get("saved_at_msec", 0))
	if saved_at_msec <= 0 or Time.get_ticks_msec() - saved_at_msec > LOBBY_PUBLISHED_MAP_CACHE_TTL_MS:
		return []
...
func _is_deleted_game_entry(entry: Dictionary) -> bool:
	...
	if _is_entry_owned_by_current_user_strict(entry) or _is_bobux_creator_entry(entry):
		UserSession.unmark_deleted_game(map_id, game_name)
		return false
...
func _scan_local_map_entries() -> Array:
	...
	while folder_name != "":
		if maps_dir.current_is_dir() and not folder_name.begins_with("."):
			...
			if not bool(entry.get("draft_unsaved", false)):
				entries.append(entry)
	...
	if is_own_profile:
		...
		for cached_entry_variant in _load_cached_cloud_map_entries(128):
			...
			if _is_deleted_game_entry(cached_entry):
				continue
			if _has_game_entry(game_entries, _get_game_key(cached_entry)):
				continue
			cached_entry["active_players"] = _get_active_player_count_for_entry(cached_entry, active_counts_by_map_id, active_counts_by_name)
			game_entries.append(cached_entry)
...
func _start_smart_play_for_game(game_info: Dictionary) -> void:
	...
	call_deferred("_increment_map_visit_async", resolved_game_info)
...
func _increment_map_visit_async(game_info: Dictionary) -> void:
	if CloudAPI == null or not CloudAPI.is_configured():
		return
	var map_id: String = str(game_info.get("map_id", "")).strip_edges()
	if map_id.is_empty():
		return
	await CloudAPI.increment_map_visits(map_id, int(game_info.get("visits", game_info.get("visits_count", 0))))
...
func _create_smart_play_card(game_info: Dictionary, show_active_players: bool, keep_compact_width: bool = false) -> Panel:
	...
	var card_width: float = 174.0 if mobile_card else 202.0
	var card_height: float = 252.0
	...
	var icon_widget := _create_game_icon_widget(game_info, 92.0 if mobile_card else 96.0)
	icon_widget.mouse_filter = Control.MOUSE_FILTER_STOP
	icon_widget.gui_input.connect(click_opens_details)
	root.add_child(icon_widget)
	...
	descriptor.text = "Active Players: %d" % int(game_info.get("active_players", 0)) if show_active_players else "Ready to continue instantly."
	descriptor.max_lines_visible = 1
	descriptor.custom_minimum_size = Vector2(0, 18)
	...
	play_button.custom_minimum_size = Vector2(0, 32)
	play_button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	play_button.size_flags_vertical = Control.SIZE_SHRINK_BEGIN
	play_button.add_theme_stylebox_override("normal", _make_primary_button_style(Color(0.12, 0.6, 0.88, 1.0)))
	play_button.add_theme_stylebox_override("hover", _make_primary_button_style(Color(0.15, 0.67, 0.96, 1.0)))
	play_button.add_theme_stylebox_override("pressed", _make_primary_button_style(Color(0.15, 0.67, 0.96, 1.0)))
	play_button.add_theme_color_override("font_color", Color.WHITE)
...
func _show_game_details_page(game_info: Dictionary) -> void:
	...
	like_btn.pressed.connect(func():
		...
		var map_id: String = str(game_info.get("map_id", "")).strip_edges()
		var updated_likes: int = like_count + 1
		game_info["likes"] = updated_likes
		like_btn.text = "Like %d" % updated_likes
		if CloudAPI != null and CloudAPI.is_configured() and not map_id.is_empty():
			await CloudAPI.like_map(map_id, like_count)

...
static func build(lobby: Control) -> void:
	...
	var sub_vp_c1 := SubViewportContainer.new()
	sub_vp_c1.set_anchors_preset(Control.PRESET_FULL_RECT)
	sub_vp_c1.stretch = true
	av_bg.add_child(sub_vp_c1)
	
	var sub_vp1 := SubViewport.new()
	sub_vp1.transparent_bg = true
	sub_vp1.own_world_3d = true
	sub_vp_c1.add_child(sub_vp1)
	
	var cam1 := Camera3D.new()
	cam1.fov = 25.0
	var cam1_pos := Vector3(0.0, 4.4, 4.2)
	var cam1_target := Vector3(0.0, 3.8, 0.0)
	cam1.transform = Transform3D(Basis.looking_at(cam1_target - cam1_pos, Vector3.UP), cam1_pos)
	sub_vp1.add_child(cam1)
	
	var dr_light1 := DirectionalLight3D.new()
	dr_light1.transform = Transform3D(Basis.looking_at(Vector3(0, 4.0, 0) - Vector3(2.0, 7.0, 3.0), Vector3.UP), Vector3(2.0, 7.0, 3.0))
	sub_vp1.add_child(dr_light1)
	
	var player_res := load("res://scenes/player/player.tscn")
	if player_res:
		var p_avatar1 = player_res.instantiate()
		p_avatar1.set("is_ui_preview", true)
		p_avatar1.set("use_local_avatar_fallback", false)
		p_avatar1.set("render_avatar_clothing_decals", true)
		p_avatar1.position.y = 0.68
		if p_avatar1.get_node_or_null("Visuals/NameLabel"):
			p_avatar1.get_node("Visuals/NameLabel").visible = false
		sub_vp1.add_child(p_avatar1)
		lobby.profile_header_preview_player = p_avatar1
		
		# Set colors directly for the mini avatar
		p_avatar1.head_color = GameState.head_color; p_avatar1.torso_color = GameState.torso_color
		p_avatar1.left_arm_color = GameState.left_arm_color; p_avatar1.right_arm_color = GameState.right_arm_color
		p_avatar1.left_leg_color = GameState.left_leg_color; p_avatar1.right_leg_color = GameState.right_leg_color
		_apply_current_session_avatar_to_preview(p_avatar1)
	...
	cam2.fov = 35.0
	var cam2_pos := Vector3(0.0, 2.7, 8.5)
	var cam2_target := Vector3(0.0, 2.7, 0.0)
	...
	if player_res:
		...
		p_avatar2.unique_name_in_owner = true

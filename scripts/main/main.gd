...
func _build_network_loading_overlay(hud: CanvasLayer) -> void:
	...
	_network_loading_overlay.color = Color(0.89, 0.9, 0.93, 0.98)
	_network_loading_overlay.modulate.a = 0.0
	_network_loading_overlay.set_anchors_preset(Control.PRESET_FULL_RECT)
	hud.add_child(_network_loading_overlay)
	hud.move_child(_network_loading_overlay, hud.get_child_count() - 1)

	_network_loading_card = Panel.new()
	_network_loading_card.set_anchors_preset(Control.PRESET_CENTER)
	_network_loading_card.offset_left = -250.0
	_network_loading_card.offset_top = -98.0
	_network_loading_card.offset_right = 250.0
	_network_loading_card.offset_bottom = 98.0
	var card_style := StyleBoxFlat.new()
	card_style.bg_color = Color(1, 1, 1, 0.98)
	...
	_network_loading_card.pivot_offset = Vector2(250.0, 98.0)
	...
	_network_loading_card.add_child(accent_bar)

	var center_box := VBoxContainer.new()
	center_box.set_anchors_preset(Control.PRESET_CENTER)
	center_box.offset_left = -210.0
	center_box.offset_top = -48.0
	center_box.offset_right = 210.0
	center_box.offset_bottom = 66.0
	...
	_network_loading_title.add_theme_font_size_override("font_size", 30)
	_network_loading_title.add_theme_color_override("font_color", Color(0.14, 0.15, 0.18, 1.0))
	...
	_network_loading_subtitle.add_theme_font_size_override("font_size", 14)
	_network_loading_subtitle.add_theme_color_override("font_color", Color(0.4, 0.42, 0.46, 1.0))
	...
	_network_loading_transport.add_theme_color_override("font_color", Color(0.42, 0.51, 0.61, 1.0))
	...
	_network_loading_transport_badge.add_theme_color_override("font_color", Color(0.11, 0.48, 0.85, 1.0))
...
func _load_selected_map() -> void:
	...
	if GameState.selected_map_folder.is_empty():
		...
		return
	var path := GameState.selected_map_folder + "/map_data.json"
	if not FileAccess.file_exists(path):
		print("[Main] No map_data.json found at: " + path)
		_map_asset_urls.clear()
		_apply_mode_settings(_map_mode_settings)
		_generate_default_ground()
		return
	...
	for block_data in blocks:
		...
		if _should_apply_saved_roblox_block_material(block_data):
			...
			mat = _runtime_rbxl_material_cache.get_part_material(roblox_props)
	...
	var loaded_runtime_refs := _load_runtime_objects_into_map(data.get("runtime_objects", []), map_target, room_origin, local_room_id)
	_apply_roblox_manifest_to_runtime_root(map_target, _map_roblox_manifest, loaded_runtime_refs, room_origin, local_room_id)
...
func _load_map_into_room_runtime(room_id: String, map_folder: String, map_target: Node3D) -> Dictionary:
	...
	var data: Dictionary = _read_map_data_dictionary(clean_map_folder)
	if data.is_empty():
		return {
			"ok": false,
	...
	for block_data in blocks:
		...
		if blocks_loaded % MAP_LOAD_BLOCKS_PER_FRAME == 0:
			await get_tree().process_frame
	var loaded_runtime_refs := _load_runtime_objects_into_map(data.get("runtime_objects", []), map_target, Vector3.ZERO, clean_room_id)
	_apply_roblox_manifest_to_runtime_root(map_target, _room_runtime_roblox_manifests.get(clean_room_id, {}), loaded_runtime_refs, Vector3.ZERO, clean_room_id)
	return {
		"ok": true,
		"room_id": clean_room_id,
		"map_folder": clean_map_folder
...
func _confirm_room_join(payload: Dictionary) -> void:
	...
	await _load_selected_map()
	_show_network_loading("Loading World...", "Waiting for your character.", 0.94)
...
func _apply_roblox_manifest_to_runtime_root(map_target: Node3D, manifest_variant: Variant, loaded_runtime_refs: Dictionary,
	...
	map_target.set_meta("roblox_manifest", manifest.duplicate(true))
	...
	_install_roblox_manifest_data_model(manifest, map_target)
	_load_manifest_scripts_into_map(manifest, loaded_runtime_refs, map_target, origin_offset, room_id)
	_refresh_runtime_roblox_ui(manifest, room_id)
...
func _install_roblox_manifest_data_model(manifest: Dictionary, context_node: Node) -> void:
	if LuaScriptEngine == null or not LuaScriptEngine.has_method("install_roblox_manifest"):
		return
	var result: Dictionary = LuaScriptEngine.install_roblox_manifest(manifest, context_node)
	if not bool(result.get("ok", false)):
...
func _refresh_runtime_roblox_ui(manifest: Dictionary, room_id: String) -> void:
	...
	if gui_host == null:
		return
	var gui_root := RobloxGuiRuntime.apply_manifest(gui_host, manifest, {
	...
	if gui_root != null and LuaScriptEngine != null and LuaScriptEngine.has_method("bind_gui_controls"):
...
func _start_runtime_lua_script_if_needed(runtime_node: Node, data: Dictionary) -> void:
	...
	if lua_source.is_empty():
		return
	if LuaScriptEngine == null or not LuaScriptEngine.has_method("run_script"):
		return
	call_deferred("_run_runtime_lua_script_deferred", runtime_node, lua_source)

func _run_runtime_lua_script_deferred(runtime_node: Node, lua_source: String) -> void:

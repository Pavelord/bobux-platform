...
@onready var collision_feet: CollisionShape3D = get_node_or_null("CollisionFeet") as CollisionShape3D
...
func _run_local_movement(delta: float) -> void:
	...
	if on_floor and not jump_pressed and Time.get_ticks_msec() >= _seat_exit_ready_at_msec:
		...
		if nearby_seat != null:
			...
			return
	if not on_floor and world_move_direction.length_squared() > 0.001 and _try_begin_ledge_mantle(world_move_direction, jump_pressed):
		_update_ledge_mantle(delta)
		return
...
func _update_character_state(is_sprinting: bool) -> void:
	...
	elif _climbing_active or _ledge_mantle_active:
...
func _update_climbing_movement(delta: float, input_vector: Vector2, world_move_direction: Vector3, camera_basis: Basis, jump_pressed: bool) -> bool:
	...
	elif not climb_hit.is_empty() and input_vector.y < -0.08:
...
func _try_begin_ledge_mantle(direction: Vector3, jump_pressed: bool) -> bool:
	if _ledge_mantle_active or _climbing_active or _swimming_active or _seated_part != null:
		return false
	if not jump_pressed and velocity.y < -8.0:
		return false
	...
	_ledge_mantle_target = (
		Vector3(top_position.x, top_position.y + _world_units(0.08), top_position.z)
		+ flat_direction * _world_units(0.42)
	)
...
func _update_animation_state(delta: float) -> void:
	...
	if _character_state == CharacterState.SWIMMING:
		...
		return
	if _climbing_active or _ledge_mantle_active:
...
func _apply_avatar_attachment_source_fallback_colors(loaded_model: Node, fallback_parts: Array) -> void:
	...
	for index in range(meshes.size()):
		...
		if part_data.is_empty() or not _avatar_attachment_part_has_color(part_data):
			continue
		if not _avatar_attachment_mesh_needs_fallback_color(mesh_instance):
			continue
		var fallback_material := _avatar_attachment_material(_avatar_attachment_part_color_value(part_data))
...
func _sync_humanoid_runtime_properties(grounded_now: bool) -> void:
	...
	humanoid.set_meta("FloorMaterial", "Air" if not grounded_now and not _climbing_active else ("Climbable" if _climbing_active else "Plastic"))
...
func _spawn_death_fragments() -> void:
	...
	for source in source_meshes:
		...
		if outward.length_squared() <= 0.01:
			outward = Vector3(randf_range(-1.0, 1.0), 0.4, randf_range(-1.0, 1.0))
		fragment.linear_velocity = outward.normalized() * randf_range(4.5, 7.5) + Vector3.UP * randf_range(3.0, 6.0)
		fragment.angular_velocity = Vector3(randf_range(-5.0, 5.0), randf_range(-5.0, 5.0), randf_range(-5.0, 5.0))
...
func _fit_collision_shapes_to_visuals() -> void:
	...
	_fit_box_collision_from_bounds(collision_feet, _combine_mesh_bounds([left_leg_mesh, right_leg_mesh]), HITBOX_FEET_PADDING, HITBOX_FEET_MIN, HITBOX_FEET_MAX, true, true)
	_fit_box_collision_from_bounds(collision_torso, _combine_mesh_bounds([torso_mesh]), HITBOX_TORSO_PADDING, HITBOX_TORSO_MIN, HITBOX_TORSO_MAX, true, true)
	_fit_box_collision_from_bounds(collision_head, _combine_mesh_bounds([head_mesh]), HITBOX_HEAD_PADDING, HITBOX_HEAD_MIN, HITBOX_HEAD_MAX, true, true)
	_fit_box_collision_from_bounds(collision_left_arm, _combine_mesh_bounds([left_arm_mesh]), HITBOX_ARM_PADDING, HITBOX_ARM_MIN, HITBOX_ARM_MAX, false, true)
	_fit_box_collision_from_bounds(collision_right_arm, _combine_mesh_bounds([right_arm_mesh]), HITBOX_ARM_PADDING, HITBOX_ARM_MIN, HITBOX_ARM_MAX, false, true)
...
func _configure_collision_profile() -> void:
	if _is_preview_instance():
		_set_collision_enabled(false)
		return
	# Every avatar owns a physical capsule. The locally controlled/server body
...
func _set_collision_enabled(enabled: bool) -> void:
	...
	var has_primary_body := collision_body != null
	if collision_body != null:
		collision_body.disabled = not enabled
	for collision_shape in [collision_feet, collision_torso, collision_head, collision_left_arm, collision_right_arm]:
		if collision_shape != null:
			collision_shape.disabled = true if has_primary_body else not enabled

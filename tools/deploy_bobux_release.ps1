...
param(
	...
	[int]$Build             = 41,
	[string]$MobileVersion  = "0.1.28-mobile",
	[int]$MobileBuild       = 29,
	[string]$LauncherVersion = "0.1.12",
	[int]$LauncherBuild     = 14,
	...
	[string]$BobuxServiceKey = $env:BOBUX_SERVICE_KEY,
...
$gameExeSha256 = (Get-FileHash $gameExe -Algorithm SHA256).Hash
$gameExeSize = (Get-Item -LiteralPath $gameExe).Length
$rbxlConverterSha256 = (Get-FileHash $rbxlConverterSource -Algorithm SHA256).Hash
$rbxlConverterSize = (Get-Item -LiteralPath $rbxlConverterSource).Length
$gameLuaExtensionSha256 = (Get-FileHash $gameLuaExtensionDll -Algorithm SHA256).Hash
$gameLuaExtensionSize = (Get-Item -LiteralPath $gameLuaExtensionDll).Length
$embeddedPck = -not (Test-Path -LiteralPath $gamePck -PathType Leaf)
$gameVersionMarker = @{
	version    = $Version
	build      = $Build
	arch       = $WindowsArchitecture
	executable_size = $gameExeSize
	executable_sha256 = $gameExeSha256
	updated_at = (Get-Date).ToUniversalTime().ToString("s") + "Z"
}

function Invoke-CompressArchiveWithRetry([string]$SourceGlob, [string]$DestinationPath, [string]$Label) {
	$lastError = $null
	for ($attempt = 1; $attempt -le 6; $attempt++) {
		try {
			...
			Compress-Archive -Path $SourceGlob -DestinationPath $DestinationPath -CompressionLevel Optimal -ErrorAction Stop
			return
		} catch {
			$lastError = $_
			if ($attempt -lt 6) {
				Write-Host "  $Label is still locked; retrying in 2 seconds ($attempt/6)..." -ForegroundColor DarkYellow
				Start-Sleep -Seconds 2
			}
		}
	}
	throw "$Label could not be compressed after 6 attempts: $($lastError.Exception.Message)"
}
...
Invoke-CompressArchiveWithRetry "$windowsStageDir\*" $windowsVersionedZip "Windows release"
...
if (Test-Path $launcherExe) {
	...
	Invoke-CompressArchiveWithRetry "$launcherExportDir\*" $launcherZip "Windows launcher"
...
$manifestObj = @{
	...
	sha256     = $winSha256
	package_size = (Get-Item -LiteralPath $windowsVersionedZip).Length
	executable = "Bobux.exe"
	executable_size = $gameExeSize
	executable_sha256 = $gameExeSha256
	embedded_pck = $embeddedPck
	required_files = @(
		@{
			path = "addons/rbxl_importer/rbxl_converter.py"
			size = $rbxlConverterSize
			sha256 = $rbxlConverterSha256
		},
		@{
			path = "addons/luaAPI/bin/$(Split-Path -Leaf $gameLuaExtensionDll)"
			size = $gameLuaExtensionSize
			sha256 = $gameLuaExtensionSha256
		}
	)
	compat_executables = @()
	mirrors    = @(
...
if ([string]::IsNullOrWhiteSpace($BobuxServiceKey)) {
	throw "Set BOBUX_SERVICE_KEY or provide -BobuxServiceKey before deploying."
}
...
$remoteScript = "#!/usr/bin/env bash`n" + ($remoteCommands -join "`n") + "`n"
$remoteInstaller = Join-Path $distDir "bobux-release-install-build$Build.sh"
[System.IO.File]::WriteAllText($remoteInstaller, $remoteScript, [System.Text.UTF8Encoding]::new($false))
scp $remoteInstaller "${Server}:/tmp/bobux-release-install.sh"
if ($LASTEXITCODE -ne 0) {
	throw "Failed to upload the remote release installer (scp exit code $LASTEXITCODE)."
}
ssh $Server "bash /tmp/bobux-release-install.sh"
if ($LASTEXITCODE -ne 0) {
	throw "Remote release installation failed (ssh exit code $LASTEXITCODE)."
}

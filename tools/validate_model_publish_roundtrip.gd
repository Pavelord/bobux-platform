...
func _validate_downloaded_glb(bytes: PackedByteArray) -> bool:
	...
	var generated_scene: Node = document.generate_scene(state) if ok else null
	var colors: Array[Color] = []
	if generated_scene != null:
		_collect_scene_surface_colors(generated_scene, colors)
	var red_found := _contains_close_color(colors, Color(0.86, 0.12, 0.08))
	var blue_found := _contains_close_color(colors, Color(0.08, 0.25, 0.88))
	if generated_scene != null:
		generated_scene.free()
	_remove_if_present(path)
	return ok and red_found and blue_found

func _collect_scene_surface_colors(node: Node, colors: Array[Color]) -> void:
	...
	for child in node.get_children():
		_collect_scene_surface_colors(child, colors)

func _contains_close_color(colors: Array[Color], expected: Color) -> bool:
	for color in colors:
		if absf(color.r - expected.r) <= 0.04 \
			and absf(color.g - expected.g) <= 0.04 \
			and absf(color.b - expected.b) <= 0.04:
			return true
	return false

...
func _initialize() -> void:
	...
	var collision_ok: bool = player.collision_layer == 2 and (player.collision_mask & 2) != 0 and player.safe_margin <= 0.04
	...
	for _frame in range(3):
		await physics_frame
	var ledge_started: bool = player.call("_try_begin_ledge_mantle", Vector3.FORWARD, true)
	...
	for _frame in range(90):
		...
		if not bool(player.get("_ledge_mantle_active")):
			break
	var ledge_ok: bool = ledge_started and not bool(player.get("_ledge_mantle_active")) and player.global_position.y > ledge_start_y + 1.0

...
func _initialize() -> void:
	...
	print("[validate_new_studio_rbxl_import] parts=%d exact_meshes=%d proxy=%d missing=%d imported_scripts=%d manifest_scripts=%d grid_visible=%s" % [
	...
	var ok := int(stats.get("parts", 0)) > 0 and int(stats.get("proxy", 0)) == 0 and int(stats.get("missing", 0)) == 0 and not bool(stats.get("grid_visible", false))

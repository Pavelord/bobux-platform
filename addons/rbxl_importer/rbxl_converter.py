...
def sanitize_for_strict_json(value, warnings, path="$", stats=None):
    ...
    if isinstance(value, dict):
        ...
        for key, child in value.items():
            safe_key = str(key)
            cleaned[safe_key] = sanitize_for_strict_json(child, warnings, "%s.%s" % (path, safe_key), stats)
        return cleaned
    if isinstance(value, (list, tuple)):
        return [
            sanitize_for_strict_json(child, warnings, "%s[%d]" % (path, index), stats)
        ...
        for item in payload.get("data", []):
            ...
            try:
                ...
                if _is_roblox_thumbnail_placeholder(image_bytes):
                    _mark_asset_failed(cache_dir, asset_id)
                    failed += 1
                elif image_bytes.startswith(b"\x89PNG") or image_bytes.startswith(b"\xff\xd8") or image_bytes.startswith(b"RIFF"):

...
static func clear_from(parent: Node, root_name: String = DEFAULT_ROOT_NAME) -> void:
	...
	for child in parent.get_children():
		if child.name == root_name or child.is_in_group(DEFAULT_GROUP_NAME):
			child.set_meta("bobux_gui_build_cancelled", true)
			child.queue_free()
...
static func apply_manifest(parent: Node, manifest_variant: Variant, options: Dictionary = {}) -> Control:
	...
	root.set_meta("bobux_gui_build_cancelled", false)
...
static func apply_manifest_async(parent: Node, manifest_variant: Variant, options: Dictionary = {}, batch_size: int = 96) -> Control:
	if parent == null:
		return null
	var runtime_options := options.duplicate(true)
	var root_name := str(options.get("root_name", DEFAULT_ROOT_NAME))
	...
	var tool_count := 0
	if bool(runtime_options.get("draw_gui", true)):
		gui_count = await _build_gui_tree_async(root, manifest, runtime_options, batch_size)
	if not _runtime_root_alive(root):
		return null
	if bool(runtime_options.get("draw_tools", true)):
		tool_count = _build_tool_hotbar(root, manifest, runtime_options)
	if not _runtime_root_alive(root):
...
static func _build_gui_tree_async(root: Control, manifest: Dictionary, options: Dictionary, batch_size: int) -> int:
	if not _runtime_root_alive(root):
		return 0
	var gui_list: Array = manifest.get("gui", []) if manifest.get("gui", []) is Array else []
	if gui_list.is_empty():
		return 0
	var tree := root.get_tree() if root.is_inside_tree() else null
	...
	for entry_variant in gui_list:
		if entry_variant is Dictionary:
			...
			if not ref.is_empty() and _belongs_to_starter_gui_entry(entry, ref, parent_by_ref, services_by_ref):
	...
	while built.size() < gui_by_ref.size() and pass_count < 16:
		...
		for ref_variant in gui_by_ref.keys():
			...
			if not _runtime_root_alive(parent_control):
				control.free()
				return built_visual_count
		...
		if not progressed:
			break
		pass_count += 1
	if _runtime_root_alive(root):
		await _apply_gui_decorators_async(root, gui_by_ref, parent_by_ref, options, safe_batch_size)
	return built_visual_count


static func _apply_gui_decorators_async(root: Control, gui_by_ref: Dictionary, parent_by_ref: Dictionary,
		options: Dictionary, batch_size: int) -> void:
	if not _runtime_root_alive(root):
		return
	var tree := root.get_tree() if root.is_inside_tree() else null
	...
	var decorators_by_class: Dictionary = {}
	var processed := 0
	# Index the manifest once. The previous implementation scanned every GUI
	# entry once per decorator class, turning large imported interfaces into an
	# O(classes * entries) startup operation with hundreds of needless frames.
	for ref_variant in gui_by_ref.keys():
		if not _runtime_root_alive(root):
			return
		var entry: Dictionary = gui_by_ref[ref_variant]
		var entry_class := str(entry.get("class", ""))
		if entry_class in apply_order:
			if not decorators_by_class.has(entry_class):
				decorators_by_class[entry_class] = []
			(decorators_by_class[entry_class] as Array).append(str(ref_variant))
		processed += 1
	...
	for target_class in apply_order:
		var refs: Array = decorators_by_class.get(target_class, []) as Array
		for ref_variant in refs:
			if not _runtime_root_alive(root):
				return
			var ref := str(ref_variant)
			var entry: Dictionary = gui_by_ref[ref]
			var parent_ref := _normalize_ref(entry.get("parent_ref", parent_by_ref.get(ref, "")))
			var target := root.find_child(_node_name_for_ref(parent_ref), true, false) as Control if not parent_ref.is_empty() else null
			if target != null:
				var props: Dictionary = entry.get("properties", {}) if entry.get("properties", {}) is Dictionary else {}
				match target_class:
					"UICorner": _apply_corner_to_control(target, props)
					"UIStroke": _apply_stroke_to_control(target, props)
					"UIScale": target.scale *= float(props.get("Scale", 1.0))
					"UIAspectRatioConstraint": _apply_aspect_ratio_constraint_to_control(target, props)
					"UISizeConstraint": _apply_size_constraint_to_control(target, props)
					"UITextSizeConstraint": _apply_text_size_constraint_to_control(target, props)
					"UIPadding": _apply_padding_to_control(target, props)
					"UIListLayout": _apply_list_layout_to_control(target, props)
					"UIGridLayout": _apply_grid_layout_to_control(target, props)
					"UIGradient": _apply_gradient_tint_to_control(target, props)
			processed += 1
			if tree != null and processed >= batch_size:
				processed = 0
				await tree.process_frame
...
static func _runtime_root_alive(root_variant: Variant) -> bool:
	# A queued Control can be freed while this async builder is suspended. Keep
	# the guard untyped so passing that stale reference never triggers a typed
	# argument conversion before is_instance_valid() can reject it.
	if not is_instance_valid(root_variant):
		return false
	var root := root_variant as Control
	return (
		root != null
		and not root.is_queued_for_deletion()
		and root.is_inside_tree()
		and not bool(root.get_meta("bobux_gui_build_cancelled", false))
	)
...
static func _apply_gradient_tint_to_control(control: Control, props: Dictionary) -> void:
	if not props.has("Transparency"):
		return
	# Roblox stores UIGradient.Transparency as a NumberSequence, not a scalar.
	# Calling float(Array) raised once per gradient and could flood a large
	# imported place with thousands of runtime errors.
	var transparency := clampf(_number_sequence_average(props.get("Transparency", 0.0)), 0.0, 1.0)
	if transparency <= 0.0:
		return
	control.modulate.a *= 1.0 - transparency


static func _number_sequence_average(value: Variant) -> float:
	if value is float or value is int:
		return float(value)
	if value is Dictionary:
		var point: Dictionary = value
		return float(point.get("value", point.get("Value", 0.0)))
	if not (value is Array):
		return 0.0
	var points: Array = value
	if points.is_empty():
		return 0.0
	if points.size() == 1 and points[0] is Dictionary:
		return float((points[0] as Dictionary).get("value", (points[0] as Dictionary).get("Value", 0.0)))
	var area := 0.0
	var covered := 0.0
	for index in range(points.size() - 1):
		if not (points[index] is Dictionary) or not (points[index + 1] is Dictionary):
			continue
		var left: Dictionary = points[index]
		var right: Dictionary = points[index + 1]
		var left_time := clampf(float(left.get("time", left.get("Time", 0.0))), 0.0, 1.0)
		var right_time := clampf(float(right.get("time", right.get("Time", left_time))), left_time, 1.0)
		var span := right_time - left_time
		if span <= 0.0:
			continue
		var left_value := clampf(float(left.get("value", left.get("Value", 0.0))), 0.0, 1.0)
		var right_value := clampf(float(right.get("value", right.get("Value", left_value))), 0.0, 1.0)
		area += (left_value + right_value) * 0.5 * span
		covered += span
	if covered <= 0.0:
		return 0.0
	return area / covered

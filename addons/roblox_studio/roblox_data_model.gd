...
func _initialize() -> void:
	...
	for service_name in DEFAULT_SERVICES:
		if service_name == "Workspace":
			continue
		var registered_variant: Variant = _services.get(service_name, null)
		if is_instance_valid(registered_variant) and not (registered_variant as Node).is_queued_for_deletion():
			continue
		_services.erase(service_name)
		var existing := get_node_or_null(NodePath(service_name))
	...
	lighting = _valid_service_node("Lighting") as Node3D
	starter_gui = _valid_service_node("StarterGui") as CanvasLayer
	starter_player = _valid_service_node("StarterPlayer")
...
func get_service(service_name: String) -> Node:
	if service_name == "Workspace":
		return workspace_alias if is_instance_valid(workspace_alias) else null
	_initialize()
	return _valid_service_node(service_name)


func has_service(service_name: String) -> bool:
	if service_name == "Workspace":
		return is_instance_valid(workspace_alias)
	_initialize()
	return _valid_service_node(service_name) != null
...
func ensure_service(service_name: String) -> Node:
	...
	if service_name == "Workspace":
		return workspace_alias if is_instance_valid(workspace_alias) else null
	var existing := _valid_service_node(service_name)
	if existing != null:
		return existing
	var node := _create_service_node(service_name)
	add_child(node, true)
...
func _valid_service_node(service_name: String) -> Node:
	var service_variant: Variant = _services.get(service_name, null)
	if not is_instance_valid(service_variant):
		_services.erase(service_name)
		return null
	var service := service_variant as Node
	if service == null or service.is_queued_for_deletion():
		_services.erase(service_name)
		return null
	return service
...
func clear_services() -> void:
	...
	for service_name in _services.keys():
		if service_name == "Workspace":
			continue
		var service := _valid_service_node(str(service_name))
		if service == null:
			continue

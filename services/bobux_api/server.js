...
async function main() {
  ...
  app.post("/api/rest/v1/rpc/fetch_my_creations", express.json({ limit: "5mb" }), async (req, res) => {
    try {
      ...
      const maps = (await listRows("maps")).filter((row) => row.owner_id === userId);
      const models = (await listRows("model_assets")).filter((row) => row.owner_id === userId);
      const avatarItems = (await listRows("avatar_items")).filter((row) => row.owner_id === userId);
      res.json({
  ...
  app.post("/api/rest/v1/rpc/like_catalog_asset", express.json({ limit: "5mb" }), async (req, res) => {
    try {
      ...
      if (existing) return res.json({ already_liked: true, likes_count: await getLikesCountForTarget(targetType, targetId) });
      ...
      const likesCount = await incrementLikesCount(targetType, targetId);
  ...
  app.post("/api/rest/v1/rpc/delete_own_maps_by_name_and_cleanup", express.json({ limit: "5mb" }), async (req, res) => {
    ...
    const rows = (await listRows("maps")).filter((row) => row.name === targetName && (!user || row.owner_id === user.record.id));
  ...
  app.post("/api/rest/v1/rpc/delete_own_map_and_cleanup", express.json({ limit: "5mb" }), async (req, res) => {
    ...
    const rows = (await listRows("maps")).filter((row) => row.id === targetId && (!user || row.owner_id === user.record.id));
    const deleted = await deleteRowsAndStorage(rows);
    res.json({ deleted_count: deleted });
  });

...
func apply_cloud_profile_payload(next_avatar_data: Variant, next_inventory_items: Variant) -> void:
	if next_avatar_data is Dictionary:
		avatar_data = _sanitize_avatar_data_for_runtime(_deserialize_colors(next_avatar_data as Dictionary))
...
func mark_deleted_game(map_id: String = "", _game_name: String = "") -> void:
	var clean_map_id: String = map_id.strip_edges()
	var changed: bool = false
...
func is_deleted_game(map_id: String = "", _game_name: String = "") -> bool:
	var clean_map_id: String = map_id.strip_edges()
	if not clean_map_id.is_empty():
		return deleted_map_ids.has(clean_map_id)
	return false
